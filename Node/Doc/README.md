...
- 性能方面
    - 当然C语言的性能是最高的
    - 得益于V8性能的Node,在调用C/C++扩展模块时，性能也是顶尖的
##### 异步编程的难点
- Node借助异步I/O模型及V8高性能引擎，突破了单线程的瓶颈，让JS在后端达到了实用价值
###### 异常处理
- Node处理异常上的约定
    - 将异常作为回调函数第一个实参传回
    - 若传回的为空值，则表明没有异常抛出
...
```
###### 阻塞代码
- 因为在JavaScript中，并没有sleep()这种线程沉睡功能
- 所以要实现阻塞后续的代码持续执行，使用如下这种很糟糕的方式
```javascript
var str = new Date();
...
// 需要阻塞的代码
```
- 这段代码会持续占用CPU判断，破坏了时间循环调度
- 因为Node是单线程，因此CPU会持续服务这段代码，对其他请求不会做出响应
#### 异步编程的解决方案
##### 事件发布/订阅模式
